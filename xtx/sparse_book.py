...
class SparseBook():
    def __init__(self, book_dict: List[Dict[float, int]], ask: bool, aligned: bool=False, n_jobs:int=1):
    ...
    def cutout(self, n_operations: int, fillna: bool=False):
        if self.n_jobs == 1:
            cutout_records = [self._cutout_record(record, n_operations, fillna) for record in self.book_dict]
        else:
...
class SparseBookFactory():
    ...
    @cached_property
    def ask_book(self):
        return SparseBook(self.make_book(self.df.iloc[:, :30]), ask=True, n_jobs=self.n_jobs)
    
    @cached_property
    def bid_book(self):
        return SparseBook(self.make_book(self.df.iloc[:, 30:60]), ask=False, n_jobs=self.n_jobs)
