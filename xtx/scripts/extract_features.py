...
def add_flatten_features(
    flatten_df: pd.DataFrame, features: pd.DataFrame, n: int, prefix: str
):
...
def extract_flatten_features(
    data: pd.DataFrame, n: int, fillna: bool
) -> pd.DataFrame():
    """
    Args:
    ...
    with Timer() as interaction_time:
        ...
        features[f"flatten_spread_{n}_median"] = (
            features[f"ask_flatten_median_{n}"] / features[f"bid_flatten_median_{n}"]
            - 1
        )
...
def main():
    ...
    target_col = "y"
    for n_per_row in (5, 10, 15, 25, 50, 100, 200, 300):
        ask_flatten_df, bid_flatten_df, features = extract_flatten_features(
            data, n=n_per_row, fillna=False
        )
